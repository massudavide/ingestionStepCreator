package sqoopConfigFile
import java.nio.file.{Paths, Files}
import java.nio.charset.StandardCharsets

import sqoopConfigFile.sqoopAux.{populatesFieldMapColumnsJava, getFirstPrimaryKey}
...
object createSqoopConfigFile {
  ...
  def main(DDLToList: List[String], databaseName: String, sourceSystemName: String, tableName: String,
    ...
    sqoopConfString += "querySource=\"SELECT t.*, '$(date '+%Y-%m-%d %H:%M:%S')' AS d_caricamento FROM " + databaseName + "." + tableName +" t WHERE 1=1 and \\$CONDITIONS\"\n"
